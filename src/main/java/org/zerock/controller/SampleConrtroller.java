...
@Controller // servle-context.xml에서 <context:component-scan base-package="org.zerock.controller" />용
@RequestMapping("/sample/*") // http://192.168.111.104:80/controller/sample/모든것에 반응
@Log4j2 // resource 폴더에 log4j2.xml필수
public class SampleConrtroller {
	
	@RequestMapping("") // http://192.168.111.104:80/controller/
	public void basic() {
		
		log.info("SampleConrtroller.basic() 메서드 실행................");
		log.info("리턴이 void이기 때문에 basic.jsp파일을 찾습니다.");
		log.info("현재 /WEB-INF/views/basic.jsp가 없음으로 크롭에는 오류가 납니다.");
		// 리턴이 void임 : 파일 [/WEB-INF/views/sample.jsp]을(를) 찾을 수 없습니다.
		// 기본적으로 url 경로에 대한 jsp를 찾으려고 한다.
	} //basic()메서드 종료
	
	@RequestMapping(value="/basic", method = {RequestMethod.GET, RequestMethod.POST})
	// http://192.168.111.104:80/controller/sample/basic -> get메서드와 post메서드 동작
	...
	public String ex01(SampleDTO dto) { //SampleDTO dto = new SampleDTO();
		...
		return "mbcex01"; //WEB-INF/views/ex01.jsp -> servle-context.xml 이 하는일
	...
	public String ex04(SampleDTO dto,@ModelAttribute("page") int page) { // int page는 게시판에 페이징기법 예시
		//@ModelAttribute("page") 안쓰면 백앤드에는 전달되지만 프론트에는 전달 안됨.
		// 스플링에서 기본적으로 제공 되는 model 객체를 사용하게 설정을 한다.
